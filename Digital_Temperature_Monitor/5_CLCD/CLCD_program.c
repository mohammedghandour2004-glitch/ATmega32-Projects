...
void CLCD_VoidInit        ( void ){
	...
	/*
	 * The LCD starts in 8-bit mode after power-on. Send only the high
	 * nibble during this sequence until 4-bit mode is selected.
	 */
	DIO_VoidWriteHighNibbles(CLCD_DATA_PORT, 0x03);
	...
	/* Select 4-bit interface mode. */
	DIO_VoidWriteHighNibbles(CLCD_DATA_PORT, 0x02);
	CLCD_VoidSendFallingEdge();
	_delay_ms(1);
