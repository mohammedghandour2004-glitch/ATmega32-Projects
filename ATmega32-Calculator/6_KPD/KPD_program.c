...
#include "6_KPD/KPD_config.h"
#include "6_KPD/KPD_interface.h"
#include "6_KPD/KPD_private.h"
	...
	for( LOC_u8Col = 0 + KPD_COL_INIT ; LOC_u8Col < KPD_COL_END + 1 ; LOC_u8Col++ ){
		...
		for( LOC_u8Row = 0 +KPD_ROW_INIT ; LOC_u8Row < KPD_ROW_END + 1 ; LOC_u8Row++ ){
			...
			if( LOC_u8GetPressed == 0 )
			{
				...
				_delay_ms(50);   // delay for switches bouncing
...

				{
					...
					==> Hint2 : if there is no pressed buttons , it will returned NOTPRESSED (0xff) and there is not any button has this ASCII (0xff)
					*/
